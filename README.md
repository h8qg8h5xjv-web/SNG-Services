...
- **`maplibre-gl` 5.24.0 — GHSA-jrc7-96c5-q579 (critical):** обход XSS-фильтра в
  `DOM.sanitize()`. Уязвимы все версии ≤ 6.4.0; исправление есть только в 6.x
  (сейчас 6.11.2) — это major-обновление с breaking changes, отдельная задача.
  Реальный риск у нас низкий: HTML в карту попадает только из нашей строки
  атрибуции «© OpenStreetMap contributors» (`lib/maps/style.ts`), Popup /
  `setHTML` не используются, стиль задан в коде. План: перейти на 6.x в
  отдельной ветке и проверить `ProvidersMap`, `StaticMiniMap`, `AdminPickerMap`.
